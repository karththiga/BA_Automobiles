...
```{r}
# Formal normality test for vehicle price
shapiro_price <- shapiro.test(auto$price)
print(shapiro_price)
...
Normality was assessed using both a formal hypothesis test and graphical methods.

- Null hypothesis (H0): Vehicle price is normally distributed.
- Alternative hypothesis (H1): Vehicle price is not normally distributed.

The Shapiro-Wilk result should be interpreted with the plot. With moderate-to-large samples, minor deviations from normality are frequently detected as statistically significant. Since the histogram is approximately bell shaped and the sample size is large (n ≈ 500), Pearson correlation and linear regression remain acceptable and robust for this analysis.
...
# Interpretation:
The Pearson correlation coefficient indicates a weak negative linear relationship between vehicle price and curb weight. Since the p-value is less than 0.05, the null hypothesis is rejected. The regression model suggests that higher curb weight is associated with a slight reduction in price in this dataset, possibly due to interaction with age, vehicle type mix, and market segment effects.
...
correlation_summary <- data.frame(
  ...
  Direction = c("Positive", "Positive", "Negative", "Negative"),
